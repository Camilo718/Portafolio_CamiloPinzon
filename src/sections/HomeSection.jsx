...
export default function HomeSection() {
  ...
  return (
    <section id="home" style={{ background: "var(--color-bg)" }}>
      <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-8 px-4 sm:px-6 md:px-10 pt-10 md:pt-14 pb-8">
        <FadeIn direction="left">
          <p className="text-xs sm:text-sm font-medium tracking-widest mb-2 text-center md:text-left" style={{ color: "var(--color-accent)" }}>
            {t.hero.greeting}
          </p>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-semibold leading-tight mb-3 text-center md:text-left" style={{ color: "var(--color-navy)" }}>
            {PERSONAL.name}
          </h1>
          <p className="text-base sm:text-lg mb-4 min-h-[28px] text-center md:text-left" style={{ color: "var(--color-muted)" }}>
            <TypingEffect words={t.hero.typingWords} />
          </p>
          <p className="text-sm leading-relaxed max-w-md mb-6 mx-auto md:mx-0 text-center md:text-left" style={{ color: "var(--color-light)" }}>
            {t.hero.bio}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center md:items-start">
            <button
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium text-white
            ...
            <a
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-medium border-[1.5px]
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 px-4 sm:px-6 md:px-10 pb-6">
        {t.stats.map((stat, i) => (
          <FadeIn key={stat.label} delay={i * 80}>
            <div className="rounded-2xl border p-4 md:p-5 text-center shadow-sm"
              style={{ background: "var(--color-bg-card)", borderColor: "var(--color-tag-bg)" }}>
              <p className="text-2xl sm:text-3xl md:text-4xl font-bold" style={{ color: "var(--color-navy)" }}>
                <AnimatedCounter value={stat.value} suffix={stat.suffix} />
              </p>
              <p className="text-xs sm:text-sm mt-1" style={{ color: "var(--color-muted)" }}>{stat.label}</p>
            </div>
          </FadeIn>
        ))}
      </div>
